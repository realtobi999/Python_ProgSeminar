...
**Knihovny** jsou soubory s kódém, které obsahují předpřipraven*é funkce, třídy nebo moduly, které můžeme znovu použít ve vlastních projektech. Je to další způsob **jak organizovat kód**, aby byl znovupoužitelný a snadno udržovatelný. Oproti jmenným prostorům jsou knihovny rozsáhlejší, obvykle pokrývají větší část aplikace nebo poskytují komplexní sadu funkcí.

### Knihovny stažené přes `pip` vs. vlastní knihovny

| **Přes `pip` (externí knihovny)**            | **Vlastní knihovny (interní)**               |
|----------------------------------------------|----------------------------------------------|
| Jsou distribuovány z Python Package Indexu (PyPI). | Píšeme si je sami pro vlastní projekty.     |
| Instalace v příkazovém řádku pomocí příkazu: `pip install numpy`.  | Jsou to vlastní seskupené moduly (složky s `__init__.py`). |
| Obsahují kód napsaný někým jiným a řeší konkrétní problém (např. zpracování dat, AI). | Mohou být tvořeny kvůli oddělení logiky nebo opakovanému použití vlastního kódu. |

## Formátování

Podstatou formátování kódu je udržet jeho zápis **konzistentní a snadno čitelný** – aby byl přehledný na první pohled. Dobré formátování zjednodušuje porozumění kódu nejen nám, ale i ostatním, kteří s ním pracují.

Formátování je často subjektivní záležitost a v rámci jednoho programovacího jazyka může existovat několik různých stylů formátování. Důležité ale je držet se jednoho stylu napříč celým projektem, aby kód působil jednotně.

Častou debatou bývá formát názvu proměnných: `snake_case`, `camelCase`, `PascalCase`. Dalšími případy mohou být komentáře, odsazování (tabulátor nebo mezery), prostor mezi blokama kódu nebo délka řádku.

Nejčastěji používaným formátovačem pro Python je `black`. Instalace probíha skrz pip příkazem `pip install black`. Samotné formátování jednotlivého souboru můžeme provést ve VsCode pomocí klávesové zkratky `Ctrl+Shift+i` nebo můžeme formátovat všechny .py soboury v projektu pomocí příkazu `black .`
